...
#define _tmp_32_type 1192
#define _loop0_33_type 1193
#define _loop1_34_type 1194
#define _loop0_36_type 1195
#define _gather_35_type 1196
#define _tmp_37_type 1197
#define _loop0_39_type 1198
#define _gather_38_type 1199
#define _tmp_40_type 1200
...
static stmt_ty memerr_lucas_rule(Parser *p);
...
static asdl_seq *_loop1_122_rule(Parser *p);
...
small_stmt_rule(Parser *p)
{
    ...
    { // &'memerr' memerr_lucas
        ...
        D(fprintf(stderr, "%*c> small_stmt[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "&'memerr' memerr_lucas"));
        stmt_ty memerr_lucas_var;
        if (
            _PyPegen_lookahead_with_int(1, _PyPegen_expect_token, p, 506)  // token='memerr'
            &&
            (memerr_lucas_var = memerr_lucas_rule(p))  // memerr_lucas
        )
        {
            D(fprintf(stderr, "%*c+ small_stmt[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "&'memerr' memerr_lucas"));
            _res = memerr_lucas_var;
            goto done;
        }
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s small_stmt[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "&'memerr' memerr_lucas"));
    }
    { // 'break'
        if (p->error_indicator) {
            p->level--;
        ...
        Token * _keyword;
        if (
            (_keyword = _PyPegen_expect_token(p, 507))  // token='break'
        )
        {
            D(fprintf(stderr, "%*c+ small_stmt[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "'break'"));
            Token *_token = _PyPegen_get_last_nonnwhitespace_token(p);
    ...
    { // 'continue'
        ...
        Token * _keyword;
        if (
            (_keyword = _PyPegen_expect_token(p, 508))  // token='continue'
        )
        {
            D(fprintf(stderr, "%*c+ small_stmt[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "'continue'"));
            Token *_token = _PyPegen_get_last_nonnwhitespace_token(p);
    ...
    { // &'global' global_stmt
        ...
        if (
            _PyPegen_lookahead_with_int(1, _PyPegen_expect_token, p, 509)  // token='global'
            &&
    ...
    { // &'nonlocal' nonlocal_stmt
        ...
        if (
            _PyPegen_lookahead_with_int(1, _PyPegen_expect_token, p, 510)  // token='nonlocal'
            &&
...
compound_stmt_rule(Parser *p)
{
    ...
    { // &'if' if_stmt
        ...
        stmt_ty if_stmt_var;
        if (
            _PyPegen_lookahead_with_int(1, _PyPegen_expect_token, p, 511)  // token='if'
            &&
...
global_stmt_rule(Parser *p)
{
    ...
    { // 'global' ','.NAME+
        ...
        asdl_seq * a;
        if (
            (_keyword = _PyPegen_expect_token(p, 509))  // token='global'
            &&
...
nonlocal_stmt_rule(Parser *p)
{
    ...
    { // 'nonlocal' ','.NAME+
        ...
        asdl_seq * a;
        if (
            (_keyword = _PyPegen_expect_token(p, 510))  // token='nonlocal'
            &&
...
// memerr_lucas:
//     | 'memerr' expression ['from' expression] 'as' star_target NAME '(' params? ')' &(',' | ')' | ':')
static stmt_ty
memerr_lucas_rule(Parser *p)
{
    if (p->level++ == MAXSTACK) {
        p->error_indicator = 1;
        PyErr_NoMemory();
    ...
    { // 'memerr' expression ['from' expression] 'as' star_target NAME '(' params? ')' &(',' | ')' | ':')
        ...
        D(fprintf(stderr, "%*c> memerr_lucas[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "'memerr' expression ['from' expression] 'as' star_target NAME '(' params? ')' &(',' | ')' | ':')"));
        Token * _keyword;
        Token * _keyword_1;
        Token * _literal;
        Token * _literal_1;
        ...
        expr_ty t;
        if (
            (_keyword = _PyPegen_expect_token(p, 506))  // token='memerr'
            &&
            (a = expression_rule(p))  // expression
            &&
            (b = _tmp_30_rule(p), !p->error_indicator)  // ['from' expression]
            &&
            (_keyword_1 = _PyPegen_expect_token(p, 514))  // token='as'
            &&
        ...
        {
            D(fprintf(stderr, "%*c+ memerr_lucas[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "'memerr' expression ['from' expression] 'as' star_target NAME '(' params? ')' &(',' | ')' | ':')"));
            Token *_token = _PyPegen_get_last_nonnwhitespace_token(p);
            if (_token == NULL) {
                p->level--;
                return NULL;
            ...
            int _end_col_offset = _token->end_col_offset;
            UNUSED(_end_col_offset); // Only used by EXTRA macro
            _res = _Py_Raise ( a , b , EXTRA ) ; _Py_withitem ( a , t , p -> arena );
            if (_res == NULL && PyErr_Occurred()) {
                p->error_indicator = 1;
                p->level--;
                return NULL;
        ...
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s memerr_lucas[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "'memerr' expression ['from' expression] 'as' star_target NAME '(' params? ')' &(',' | ')' | ':')"));
    }
    _res = NULL;
  done:
    p->level--;
...
import_from_as_name_rule(Parser *p)
{
    ...
    { // NAME ['as' NAME]
        ...
        if (
            (a = _PyPegen_name_token(p))  // NAME
            &&
            (b = _tmp_37_rule(p), !p->error_indicator)  // ['as' NAME]
        )
        {
...
dotted_as_name_rule(Parser *p)
{
    ...
    { // dotted_name ['as' NAME]
        ...
        if (
            ...
            &&
            (b = _tmp_40_rule(p), !p->error_indicator)  // ['as' NAME]
        )
        {
...
if_stmt_rule(Parser *p)
{
    ...
    { // 'if' named_expression ':' block elif_stmt
        ...
        stmt_ty c;
        if (
            (_keyword = _PyPegen_expect_token(p, 511))  // token='if'
            &&
...
with_item_rule(Parser *p)
{
    ...
    { // expression 'as' star_target &(',' | ')' | ':')
        ...
        if (
            ...
            &&
            (_keyword = _PyPegen_expect_token(p, 514))  // token='as'
            &&
            (t = star_target_rule(p))  // star_target
            &&
            _PyPegen_lookahead(1, _tmp_49_rule, p)
        )
        {
...
except_block_rule(Parser *p)
{
    ...
    { // 'except' expression ['as' NAME] ':' block
        ...
        if (
            ...
            (e = expression_rule(p))  // expression
            &&
            (t = _tmp_51_rule(p), !p->error_indicator)  // ['as' NAME]
            &&
            (_literal = _PyPegen_expect_token(p, 11))  // token=':'
            &&
            (b = block_rule(p))  // block
...
raise_stmt_rule(Parser *p)
{
    ...
    { // 'raise' expression ['from' expression]
        ...
        if (
            ...
            (a = expression_rule(p))  // expression
            &&
            (b = _tmp_52_rule(p), !p->error_indicator)  // ['from' expression]
        )
        {
...
function_def_raw_rule(Parser *p)
{
    ...
    { // ASYNC 'def' NAME '(' params? ')' ['->' expression] ':' func_type_comment? block
        ...
        if (
            ...
            (_literal_1 = _PyPegen_expect_token(p, 8))  // token=')'
            &&
            (a = _tmp_54_rule(p), !p->error_indicator)  // ['->' expression]
            &&
            (_literal_2 = _PyPegen_expect_token(p, 11))  // token=':'
            &&
            (tc = func_type_comment_rule(p), !p->error_indicator)  // func_type_comment?
...
class_def_raw_rule(Parser *p)
{
    ...
    { // 'class' NAME ['(' arguments? ')'] ':' block
        ...
        if (
            ...
            (a = _PyPegen_name_token(p))  // NAME
            &&
            (b = _tmp_71_rule(p), !p->error_indicator)  // ['(' arguments? ')']
            &&
            (_literal = _PyPegen_expect_token(p, 11))  // token=':'
            &&
...
slice_rule(Parser *p)
{
    ...
    { // expression? ':' expression? [':' expression?]
        ...
        if (
            ...
            &&
            (c = _tmp_96_rule(p), !p->error_indicator)  // [':' expression?]
        )
        {
...
tuple_rule(Parser *p)
{
    ...
    { // '(' [star_named_expression ',' star_named_expressions?] ')'
        ...
        if (
            (_literal = _PyPegen_expect_token(p, 7))  // token='('
            &&
            (a = _tmp_101_rule(p), !p->error_indicator)  // [star_named_expression ',' star_named_expressions?]
            &&
            (_literal_1 = _PyPegen_expect_token(p, 8))  // token=')'
        )
        {
...
args_rule(Parser *p)
{
    ...
    { // ','.(starred_expression | named_expression !'=')+ [',' kwargs]
        ...
        if (
            ...
            &&
            (b = _tmp_110_rule(p), !p->error_indicator)  // [',' kwargs]
        )
        {
...
star_targets_tuple_seq_rule(Parser *p)
{
    ...
    { // star_target ((',' star_target))+ ','?
        ...
        if (
            (a = star_target_rule(p))  // star_target
            &&
            (b = _loop1_122_rule(p))  // ((',' star_target))+
            &&
            (_opt_var = _PyPegen_expect_token(p, 12), !p->error_indicator)  // ','?
        )
        {
...
invalid_arguments_rule(Parser *p)
{
    ...
    { // expression for_if_clauses ',' [args | expression for_if_clauses]
        ...
        if (
            ...
            (_literal = _PyPegen_expect_token(p, 12))  // token=','
            &&
            (_opt_var = _tmp_126_rule(p), !p->error_indicator)  // [args | expression for_if_clauses]
        )
        {
            D(fprintf(stderr, "%*c+ invalid_arguments[%d-%d]: %s succeeded!\n", p->level, ' ', _mark, p->mark, "expression for_if_clauses ',' [args | expression for_if_clauses]"));
...
_loop1_122_rule(Parser *p)
{
    ...
    { // (',' star_target)
        ...
        D(fprintf(stderr, "%*c> _loop1_122[%d-%d]: %s\n", p->level, ' ', _mark, p->mark, "(',' star_target)"));
        void *_tmp_151_var;
        while (
        ...
        p->mark = _mark;
        D(fprintf(stderr, "%*c%s _loop1_122[%d-%d]: %s failed!\n", p->level, ' ',
                  p->error_indicator ? "ERROR!" : "-", _mark, p->mark, "(',' star_target)"));
    }
    if (_n == 0 || p->error_indicator) {
        PyMem_Free(_children);
    ...
    for (int i = 0; i < _n; i++) asdl_seq_SET(_seq, i, _children[i]);
    PyMem_Free(_children);
    _PyPegen_insert_memo(p, _start_mark, _loop1_122_type, _seq);
    p->level--;
    return _seq;
}
